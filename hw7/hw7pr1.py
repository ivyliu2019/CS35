...
def try_filter( image ):
    ...
    new_image = example_filter_inv( image )
...
# try_filter!
#
def try_filter2( image1, image2 ):
    """ try out a filter that takes in two images! """
    from matplotlib import pyplot as plt
...
def new_filter( image ):
    """ new_filter takes in a single image,
        transforms its pixels in an unusual
        way and returns the new image
    """
    new_image = image.copy()
    num_rows, num_cols, num_chans = new_image.shape
    for row in range(num_rows):
        for col in range(num_cols):
            r, g, b = image[row,col]
            new_image[row,col] = [255-g, 255-b, 255-r]
    return new_image

# In[ ]:

def two_image_filter( image1, image2 ):
    """ two_image_filter takes in two input
        images and outputs a novel image that
        incorporates information from the
        pixels of each of the inputs.
    """
    num_rows = min(image1.shape[0], image2.shape[0])
    num_cols = min(image1.shape[1], image2.shape[1])
    num_chans = 3
    new_image = np.zeros((num_rows, num_cols, num_chans), dtype=int)
    for row in range(num_rows):
        for col in range(num_cols):
            r1, g1, b1 = image1[row,col]
            r2, g2, b2 = image2[row,col]
            new_image[row,col] = [0.5*r1 + 0.5*r2, 0.5*g1 + 0.5*g2, 0.5*b1 + 0.5*b2]
            new_image = np.uint8(new_image)
    return new_image
"""
include in a triple-quoted string at the bottom of your file
a comment that briefly explains the motivation and what each
of your two filters does (a high-level, big-picture view,
since the code itself describes the low-level details :-)
"""
