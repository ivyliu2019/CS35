...
if RUN_MOVIEREVIEW_CLASSIFIER == True:
    ...
    def opinion_features(fileid):
        ...
        # check the difference between positive_count and negative_count
        ...
        subjectivity = 0
        sentimentScore = 0
        blob = TextBlob(movie_reviews.raw(fileid))
        for sentence in blob.sentences:
            polarity += sentence.sentiment.polarity
            subjectivity += sentence.sentiment.subjectivity
        sentimentScore = 0.5 * subjectivity + 0.5 * polarity
        ...
        for sentence in blob.sentences:
            ...
            for word in sentence.words:
                sentenceLength += 1
            listSentenceLength += [sentenceLength]
        averageLength = sum(listSentenceLength) * 1.0/len(listSentenceLength)

        # here is the dictionary of features...
        features = {'positive': positive_count,'negative':negative_count,\
                    'more positive': diff,'sentiment': sentimentScore, \
                    'average sentence length': averageLength}
    ...
    if PRINT_ERRORS == True:
        SE = sorted(errors)
        print("There were", len(SE), "errors:")
        print('Movie Review: guess (actual)')
        num_to_print = 10
        for (actual, guess, fileid) in SE:
            if actual == 'pos' and guess == 'neg': # adjust these as needed...
                print('  {0:>10}: {1} ({2})'.format(fileid, guess, actual))
                num_to_print -= 1
                if num_to_print == 0: break
        print()
    ...
    #       My final set of features ends up giving me a score that's above 0.67.
    #   (b) what other features you tried and which ones seemed to
    #       help the most/least
    #       I added the negative count, the difference between positive/negative
    #       count, the average length of sentences and the average sentiment score.
    #       Among all the features above, the one that semmed to help the most is
    #       the average length of sentences and the least is the average sentiment
    #       score.
    #
